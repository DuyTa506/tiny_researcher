...
> **Guide for Frontend AI Agent** to develop the Research Assistant application.
...
### Styling Rules
1.  **NO TailwindCSS**: Use Vanilla CSS + CSS Modules.
2.  **Naming Convention**: `styles.module.css` placed in the same folder as the component.
3.  **CSS Variables**: Define colors, spacing in `globals.css` and reuse.
    ```css
    :root {
      --primary: #2563eb;
      --surface: #ffffff;
...
- **Client State**: Use `useState` for local UI state. Avoid Redux/Zustand unless absolutely necessary.

### API Integration
- Use **Repository Pattern** in `src/services`.
