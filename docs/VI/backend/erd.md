# Entity Relationship Diagram - Research Assistant

## ERD Diagram

```mermaid
erDiagram
    USERS ||--o{ SOURCES : creates
    USERS ||--o{ REPORTS : owns
    USERS {
        uuid id PK
        string email
        string name
        jsonb preferences
        timestamp created_at
    }
    
    SOURCES ||--o{ PAPERS : produces
    SOURCES {
        uuid id PK
        uuid user_id FK
        string type "rss, keyword, author"
        text value
        jsonb config
        string schedule "cron format"
        timestamp last_run
        boolean active
        timestamp created_at
    }
    
    PAPERS ||--o{ PAPER_EVALUATIONS : has
    PAPERS ||--o{ PAPER_CLUSTERS : belongs_to
    PAPERS {
        uuid id PK
        uuid source_id FK
        string title
        text abstract
        text full_text
        jsonb authors
        date published_date
        string url
        string doi
        string arxiv_id
        text fingerprint "hash for dedup"
        jsonb summary "structured JSON"
        timestamp created_at
        timestamp updated_at
    }
    
    TOPICS ||--o{ PAPER_EVALUATIONS : evaluates
    TOPICS {
        uuid id PK
        uuid user_id FK
        string name
        text description
        jsonb keywords
        timestamp created_at
    }
    
    PAPER_EVALUATIONS {
        uuid id PK
        uuid paper_id FK
        uuid topic_id FK
        float relevance_score "0-10"
        text reasoning
        boolean is_relevant
        timestamp evaluated_at
    }
    
    REPORTS ||--o{ CLUSTERS : contains
    REPORTS {
        uuid id PK
        uuid user_id FK
        uuid topic_id FK
        date date_from
        date date_to
        text content "markdown"
        jsonb metadata
        string status "draft, published"
        timestamp created_at
    }
    
    CLUSTERS ||--o{ PAPER_CLUSTERS : groups
    CLUSTERS {
        uuid id PK
        uuid report_id FK
        string name
        text description
        jsonb summary
        int paper_count
        timestamp created_at
    }
    
    PAPER_CLUSTERS {
        uuid paper_id FK
        uuid cluster_id FK
        float similarity_score
        timestamp added_at
    }
    
    EMBEDDINGS {
        uuid id PK
        uuid paper_id FK
        vector embedding "pgvector or external"
        string model_name
        timestamp created_at
    }
    
    PAPERS ||--|| EMBEDDINGS : has
    
    FEEDBACK {
        uuid id PK
        uuid user_id FK
        uuid report_id FK
        uuid paper_id FK "nullable"
        uuid cluster_id FK "nullable"
        string action "upvote, downvote, flag"
        text note
        timestamp created_at
    }
    
    USERS ||--o{ FEEDBACK : provides
    REPORTS ||--o{ FEEDBACK : receives
```

## Table Descriptions

### Core Tables

#### `users`
Quản lý người dùng hệ thống.
- `preferences`: JSON chứa settings như LLM model preference, notification settings

#### `sources`
Định nghĩa các nguồn cần theo dõi.
- `type`: 
  - `rss`: RSS feed URL
  - `keyword`: Search query string
  - `author`: Track specific author
- `config`: JSON chứa thêm params (e.g., `{"max_results": 50}`)
- `schedule`: Cron expression (e.g., `"0 8 * * 1"` = every Monday 8am)

#### `papers`
Lưu trữ metadata của papers.
- `summary`: JSON structure
  ```json
  {
    "problem": "...",
    "method": "...",
    "result": "...",
    "keywords": ["a", "b"]
  }
  ```
- `fingerprint`: Hash dùng cho deduplication
  ```python
  fingerprint = sha256(title.lower() + "|" + "|".join(sorted(authors)))
  ```

#### `topics`
Chủ đề nghiên cứu của user (một user có thể theo dõi nhiều topics).

#### `paper_evaluations`
Kết quả đánh giá relevance của paper với topic.
- Quan hệ: 1 paper có thể được đánh giá với nhiều topics
- `reasoning`: LLM explanation để audit

### Reporting Tables

#### `reports`
Báo cáo được generate.
- `content`: Full markdown text
- `metadata`: 
  ```json
  {
    "total_papers": 23,
    "filters_applied": {...},
    "llm_model": "gemini-1.5-flash"
  }
  ```

#### `clusters`
Nhóm các papers theo hướng nghiên cứu.
- Mỗi report có nhiều clusters
- `summary`: Tổng quan do LLM generate

#### `paper_clusters`
Many-to-many relationship giữa papers và clusters.
- 1 paper có thể thuộc nhiều clusters (edge case)
- `similarity_score`: Confidence score

### Supporting Tables

#### `embeddings`
Vector embeddings cho semantic search.
- Có thể sử dụng:
  - **PostgreSQL pgvector extension**
  - **External Vector DB** (Qdrant, Pinecone)
- `model_name`: Track embedding model (e.g., "all-MiniLM-L6-v2")

#### `feedback`
Human-in-the-loop feedback.
- User đánh giá kết quả (để cải thiện model sau này)

## Indexes và Constraints

### Primary Indexes
```sql
-- Performance optimization
CREATE INDEX idx_papers_date ON papers(published_date DESC);
CREATE INDEX idx_papers_source ON papers(source_id);
CREATE INDEX idx_papers_fingerprint ON papers(fingerprint);

-- Full-text search
CREATE INDEX idx_papers_title_fts ON papers USING gin(to_tsvector('english', title));
CREATE INDEX idx_papers_abstract_fts ON papers USING gin(to_tsvector('english', abstract));

-- Evaluation lookup
CREATE INDEX idx_evaluations_topic ON paper_evaluations(topic_id, relevance_score DESC);

-- Report generation
CREATE INDEX idx_clusters_report ON clusters(report_id);
```

### Constraints
```sql
-- Unique constraint for deduplication
ALTER TABLE papers ADD CONSTRAINT unique_fingerprint UNIQUE(fingerprint);

-- Check constraints
ALTER TABLE paper_evaluations 
    ADD CONSTRAINT check_score 
    CHECK (relevance_score >= 0 AND relevance_score <= 10);

-- Cascading deletes
ALTER TABLE papers 
    ADD FOREIGN KEY (source_id) 
    REFERENCES sources(id) 
    ON DELETE CASCADE;
```

## Data Migration Strategy

### Version 1 (MVP)
- Core tables: `sources`, `papers`, `reports`, `clusters`, `paper_clusters`
- SQLite for local development

### Version 2 (Production)
- Add: `users`, `topics`, `paper_evaluations`
- Migrate to PostgreSQL
- Add pgvector extension

### Version 3 (Scale)
- Add: `embeddings`, `feedback`
- Separate vector storage to Qdrant/Pinecone
- Add caching layer (Redis)
