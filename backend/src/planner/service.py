...
from src.tools.registry import get_tools_description, TOOL_REGISTRY
...
class PlannerService:
    ...
    async def generate_research_plan(self, request: ResearchRequest) -> ResearchPlan:
        ...
        # Build context for LLM including user hints and available tools
        user_context = self._build_prompt_context(request)
    ...
    def _build_prompt_context(self, request: ResearchRequest) -> str:
        ...
        if request.time_window:
            parts.append(
                f"- Time window: {request.time_window.start_date} to {request.time_window.end_date}"
            )

        return "\n".join(parts)

    def _inject_user_data(
        self, steps: List[ResearchStep], request: ResearchRequest
    ) -> List[ResearchStep]:
        """
        ...
        if request.keywords and steps:
            first_research_step = next(
                (s for s in steps if s.action == "research"), steps[0]
            )
        ...
        if request.research_questions:
            ...
            deep_step = next(
                (
                    s
                    for s in steps
                    if "deep" in s.title.lower() or "specific" in s.title.lower()
                ),
                None,
            )
            if deep_step:
    ...
    def _create_fallback_plan(self, request: ResearchRequest) -> ResearchPlan:
        ...
        if request.sources:
            steps.append(
                ResearchStep(
                    ...
                    sources=request.sources,
                    tool="collect_urls",
                    tool_args={"urls": request.sources},
                    completed=False,
                )
            )
        ...
        unique_queries = list(set(initial_queries))
        steps.append(
            ResearchStep(
                id=len(steps) + 1,
                action="research",
        ...
        if request.research_questions:
            steps.append(
                ResearchStep(
                    ...
                    tool="search",
                    tool_args={
                        "query": " ".join(request.research_questions[:3]),
                        "max_results": 20,
                    },
                    completed=False,
                )
        ...
        # Step 3: Analyze (no tool - internal processing)
        steps.append(
            ResearchStep(
                id=len(steps) + 1,
                action="analyze",
        ...
        # Step 4: Synthesize (no tool - internal processing)
        steps.append(
            ResearchStep(
                id=len(steps) + 1,
                action="synthesize",
    ...
    def get_steps_by_action(
        self, plan: ResearchPlan, action: str
    ) -> List[ResearchStep]:
