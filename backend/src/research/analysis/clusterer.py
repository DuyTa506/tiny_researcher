...
class Cluster:
    def __init__(
        self, id: int, name: str, paper_indices: List[int], description: str = ""
    ):
        self.id = id
        self.name = name
...
class ClustererService:
    ...
    async def cluster_papers(
        ...
        n_clusters = min(
            len(papers) // 2 + 1, 5
        )  # Heuristic: roughly 2-3 papers per cluster, max 5
        if n_clusters < 1:
            n_clusters = 1
        ...
        for label, indices in clusters_map.items():
            ...
            results.append(
                Cluster(
                    id=int(label), name=name, paper_indices=indices, description=desc
                )
            )
    ...
    async def _generate_cluster_label(
        ...
        prompt = PromptManager.get_prompt(
            "CLUSTERER_LABELING", titles=titles, language=language
        )
        try:
