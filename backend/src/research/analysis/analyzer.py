...
class AnalyzerService:
    ...
    def __init__(
        self, llm_client: LLMClientInterface, paper_repo: PaperRepository = None
    ):
        self.llm = llm_client
    ...
    async def analyze_batch(
        ...
        for i in range(0, len(papers), self.BATCH_SIZE):
            ...
            logger.info(
                f"Analyzed batch {i//self.BATCH_SIZE + 1}, "
...
"""
        ...
        try:
            ...
            logger.debug(
                f"Parsed results type: {type(results)}, count: {len(results) if isinstance(results, list) else 'N/A'}"
            )
    ...
    async def score_and_persist(
        ...
        logger.info(
            f"Scoring complete: {len(relevant)} relevant, "
    ...
    async def detect_gaps(
        self, current_findings: List[str], original_goal: str
    ) -> List[str]:
        """
        ...
        prompt = PromptManager.get_prompt(
            "ANALYZER_GAP_DETECTION", goal=original_goal, findings=findings_text
        )
