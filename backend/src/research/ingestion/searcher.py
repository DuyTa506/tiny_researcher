    ...
    async def search(
        ...
        while retry_count <= max_retries:
            try:
                        ...
                        if response.status_code == 429:
                            ...
                            if retry_after:
                                ...
                                jitter = (
                                    base_wait
                                    * 0.2
                                    * (0.5 + asyncio.get_event_loop().time() % 0.5)
                                )
                                wait_time = base_wait + jitter
                                logger.warning(
                                    "arxiv_429_exponential_backoff",
                ...
                logger.error(
                    "arxiv_search_error", error=str(e), retry_count=retry_count
                )
    ...
    async def search(
        ...
        while retry_count <= max_retries:
            try:
                ...
                logger.error(
                    "openalex_search_error", error=str(e), retry_count=retry_count
                )
                if retry_count < max_retries:
                ...
                for article in articles[:max_results]:
                    try:
                        ...
                        title = (
                            ...
                            else f"Paper {arxiv_id}"
                        )

                        abstract_el = article.locator(
                            "p.text-gray-500, p.text-gray-600"
                        ).first
                        abstract = (
                            await abstract_el.text_content()
                        ...
                        results.append(
                            {
                                "title": (
                                    title.strip() if title else f"Paper {arxiv_id}"
                                ),
